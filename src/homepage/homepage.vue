    ...
    <div class="business">
      ...
      <div class="busInner">
        <div class="innerCell" v-for="product in objone" :key="product.id" @click="ToDetail(product.id)">
          ...
          <p class="pThrid">
            <span>￥{{product.marketPrice}}</span>{{product.unit}}</p>
        ...
        <transition name="slideleft">
          <div class="youone" v-show="busInner">
            <div class="innerCell" v-for="product in objtwo" :key="product.id" >
              ...
              <p class="pThrid">
                <span>￥{{product.marketPrice}}</span>{{product.unit}}</p>
              <a href="javascript:void(0)" class="proBtn" @click="ToDetail(product.id)">查看详情</a>
            </div>
          </div>
        </transition>
...
export default {
  methods: {
    ...
    //明星产品推荐跳列表页
    innercell() {
      this.$router.push({ path: "/inner/list1?num=1" });
    },
    //初创企业必备跳商品详情
    ToDetail(id) {
    ...
    //推荐服务商跳店铺首页
    ToShop(id){
      this.$router.push({ path: "/inner/shophome", query: { id: id } });
    }
  },
  data() {
    return {
